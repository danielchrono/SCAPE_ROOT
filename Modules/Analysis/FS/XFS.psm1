...
function Get-ScapeXFSMeta {
    ...
    if (-not $Script:C) { Initialize-ScapeXFSParser }
    ...
    $blocksize = [System.BitConverter]::ToUInt32($Buffer, $Offset + $Script:C.FS.XFS.SB_BLOCKSIZE_OFF)
    $dblocks = [System.BitConverter]::ToUInt64($Buffer, $Offset + $Script:C.FS.XFS.SB_DBLOCKS_OFF)
    $rblocks = [System.BitConverter]::ToUInt64($Buffer, $Offset + $Script:C.FS.XFS.SB_RBLOCKS_OFF)
    $agcount = [System.BitConverter]::ToUInt32($Buffer, $Offset + $Script:C.FS.XFS.SB_AGCOUNT_OFF)
    $agblocks = [System.BitConverter]::ToUInt32($Buffer, $Offset + $Script:C.FS.XFS.SB_AGBLOCKS_OFF)
    $rootino = [System.BitConverter]::ToUInt64($Buffer, $Offset + $Script:C.FS.XFS.SB_ROOTINO_OFF)
    $uuid = [System.BitConverter]::ToString($Buffer, $Offset + $Script:C.FS.XFS.SB_UUID_OFF, 16) -replace '-', ''
